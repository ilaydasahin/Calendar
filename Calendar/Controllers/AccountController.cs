    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult Register(RegisterViewModel model)
        {
            ...
            AppUser user = new AppUser()
            {
                ...
                Email = model.Email,
                Color = model.Color,
                IsOrganizator = model.IsOrganizator
            };
            ...
            if (result.Succeeded)
            {
                bool roleCheck = model.IsOrganizator ? AddRole("Organizator") : AddRole("DugunSahibi");
                if (!roleCheck)
                {
                    return View("Error");
                }
                _userManager.AddToRoleAsync(user, model.IsOrganizator ? "Organizator" : "DugunSahibi").Wait();
